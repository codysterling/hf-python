...
#natoms, atoms, coords = read_xyz("/home/cody/Documents/Gitlab/hf-python/xyz-files/heh.xyz")
...
# changed He zeta to match ORCA?
#zetas = {'H': [1.24], 'He': [1.6875], 'Li': [2.69, 0.75], 'Be': [3.68, 1.10], 'B': [4.68, 1.45, 1.45, 1.45, 1.45], 'C': [5.67, 1.72, 1.72, 1.72, 1.72], 'N': [6.67, 1.95, 1.95, 1.95, 1.95], 'O': [7.66, 2.25, 2.25, 2.25, 2.25], 'F': [8.65, 2.55, 2.55, 2.55, 2.55]}
...
for i,atom in enumerate(atoms):
	...
	for qA in range(len(zetas[atom])):
		...
		for lA in range(len(d_A)): # Iterating over contracted Gaussians for A
			...
			for j,atomB in enumerate(atoms):
				...
				for qB in range(len(zetas[atomB])):
					...
					for lB in range(len(d_B)): # Iterating over contracted Gaussians for B
						a = (i+1)*(qA+1)-1
						b = (j+1)*(qB+1)-1
						...
						for k,atomC in enumerate(atoms): # Going through C and D to make the M tensor
							...
							for qC in range(len(zetas[atomC])):
								...
								for lC in range(len(d_C)): # Iterating over contracted Gaussians for C
									...
									for m,atomD in enumerate(atoms):
										...
										for qD in range(len(zetas[atomD])):
											...
											for lD in range(len(d_D)): # Iterating over contracted Gaussians for D
												c = (k+1)*(qC+1)-1
												d = (m+1)*(qD+1)-1
...
print("Nuclear repulsion is {}".format(nuc_repul()))
print("Total energy is {}".format(E_list[-1]+nuc_repul()))
